...
public class DateUtil {
	...
	/**
	 * 以yyyy-MM-dd HH24:mm:ss格式返回系统日期时间
	 * 
	 * @return 系统日期时间
	 * @since 1.0
	 * @history
	...
	/**
	 * yyyyMMddHHmmss格式返回系统日期时间
	 * 
	 * @return 系统日期时间
	 * @since 1.0
	 * @history
	 */
	public synchronized static String getSysDateTimeString1() {
		String sysFormat = "yyyyMMddHHmmss";
		SimpleDateFormat sysDateTime = new SimpleDateFormat(sysFormat);
		return toString(new java.util.Date(System.currentTimeMillis()),sysDateTime);
	}
...
*/
	...
	/**
	 * 两个日期比较 日期格式yyyy-mm-dd hh24:mi:ss
	 * @param startDate
	 * @param endDate
	 * @return true startDate>endDate    false startDate<=endDate
	 */
	public synchronized static boolean compareDate1(String startDate,String endDate){
		try {
			java.util.Date sDate = sdfDateTime.parse(startDate);
			java.util.Date eDate = sdfDateTime.parse(endDate);
	...
	/**
	 * 从java.sql.Timestamp Object 中取出hour
	 * @return
	 */
	public synchronized static int getCurrentHour() {
		java.sql.Timestamp time = new Timestamp(System.currentTimeMillis());
		GregorianCalendar obj = DateUtil.convertTimestampToCalendar(time);
		int hour = obj.get(GregorianCalendar.HOUR_OF_DAY);
		return hour;
