 ...
 *
 */
public class UnionPaySecureUtil extends BaseClass {
	/**
	 * 算法常量： MD5
	 */
	...
	/**银联支付加解密工具类实例*/
	private volatile static UnionPaySecureUtil INSTANCE = null;
	
	/**获取银联支付加解密工具类实例*/
	public static UnionPaySecureUtil getInstance(){
		
		if(INSTANCE == null){
			
			synchronized (UnionPaySecureUtil.class) {
				
				if(INSTANCE == null){
					
					INSTANCE = new UnionPaySecureUtil();
	 ...
	 * @return 计算结果
	 */
	public byte[] md5X16(String datas, String encoding) {
	 ...
	 * @return 计算结果
	 */
	public byte[] sha1X16(String data, String encoding) {
	 ...
	 * @return
	 */
	public byte[] md5(String datas, String encoding) {
		try {
	 ...
	 * @return
	 */
	public byte[] sha1(String datas, String encoding) {
		try {
	 ...
	 * @throws Exception
	 */
	public byte[] signBySoft(PrivateKey privateKey, byte[] data)
			throws Exception {
		byte[] result = null;
	 ...
	 * @throws Exception
	 */
	public boolean validateSignBySoft(PublicKey publicKey,
	 ...
	 * @return 转PIN结果
	 */
	public String EncryptPin(String pin, String card, String encoding,
			PublicKey key) {
		/** 生成PIN Block **/
	 ...
	 * @return
	 */
	public String EncryptData(String dataString, String encoding,
			PublicKey key) {
		/** 使用公钥对密码加密 **/
		byte[] data = null;
		try {
	 ...
	 * @return 解密后的数据
	 */
	public String DecryptedData(String dataString, String encoding,
			PrivateKey key) {
		byte[] data = null;
		try {
	 ...
	 * @throws IOException
	 */
	public byte[] base64Decode(byte[] inputByte) throws IOException {
	 ...
	 * @return
	 */
	public String Hex2Str(byte[] b, int len) {
		String str = "";
	 ...
	 * @return
	 */
	public String fillString(String string, char filler, int totalLength, boolean atEnd) {
	 ...
	 * @throws Exception
	 */
	public byte[] decryptedPin(PrivateKey privateKey, byte[] cryptPin) {
	 ...
	 * @return
	 */
	private byte[] addPKCS1Padding(byte[] aBytesText, int aBlockSize) {
	...
	public String assymEncrypt(String tPIN, String iPan, RSAPublicKey publicKey) {
		...
		bLogInfo(0, "SampleHashMap::assymEncrypt([" + tPIN + "])");
		bLogInfo(0, "SampleHashMap::assymEncrypt(PIN =[" + tPIN + "])");
		...
		try {
			...
			bLogInfo(0, "SampleHashMap::assymEncrypt(EncryptCardNo =[" + tEncryptPIN + "])");
	 ...
	 * @return
	 */
	public String trace(byte[] inBytes) {
		int i, j = 0;
	 ...
	 * @param fill
	 */
	private void bytesSet(byte[] inBytes, char fill) {
		if (inBytes.length == 0) {
			return;
		}
